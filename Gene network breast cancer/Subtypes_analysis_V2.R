...
setwd("C:/Users/Fengwei Yang/Desktop/YFW work/GRA/Other paper/Breast Cancer/Results/Subtypes")
...
glm.kclass.crossvalidation <-
  function(X, Y, sepn){
    ...
    for (i in 1:sepn) {
      ...
      lowb = ceiling(n/sepn*(i-1))+1
      upb = ceiling(n/sepn*i)
      
      X_train = X[, -lowb:-upb]
      X_test = X[, lowb1:upb1]

      
      y_Train = as.vector(Y[-lowb:-upb])
      y_Test =  as.vector(Y[lowb:upb])
      
      #### mLDA fit ####
      try1  <- mLDA.Kclass(t(X_train), y_Train, t(X_test), Z=NULL, Z.new=NULL, K=4, tau=50, alpha=0.9, nu=150, d=2, nb=5)
      temp_output = length(which(try1$PredClass != y_Test))  
      valid_output = rbind(valid_output,temp_output)
    }
    return(valid_output)
  }

temp <- glm.kclass.crossvalidation(X=ExprDD, Y=Subtypes_analysis1, sepn=5)



########DE analysis
#edgeR can not be used beyond pairwise comparision 
...
ExprDD <- exprFinal_mLDA_cross
ExprDD <- ExprDD[rownames(ExprDD)%in%Subtypes_analysis$sampleID,]
temp <- as.data.frame(rownames(ExprDD))
Subtypes_analysis <- merge(temp, Subtypes_analysis, by.x ="rownames(ExprDD)", by.y = "sampleID")
Subtypes_analysis1 <- Subtypes_analysis$PAM50mRNAnature2012
try2  <- mLDA.Kclass(ExprDD, Subtypes_analysis1, ExprDD, Z=NULL, Z.new=NULL, K=4, tau=50, alpha=0.9, nu=150, d=2, nb=5)
saveRDS(try2,file=paste("Subtypes_analysis_mLDA", ".rds", sep=""))
which(is.na(ExprDD))


try2_new <- readRDS(file=paste("subtypes_mLDA_V2", ".rds", sep=""))
