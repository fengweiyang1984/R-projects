...
setwd("C:/Users/Fengwei Yang/Desktop/YFW work/GRA/Other paper/Breast Cancer/Results/Survival")
source("library_CIS_imagingData_122022.R")
##################################################################################################
#5-years survival
...
#write.csv(qlfT_sort, "survival_edgeR.csv")
...
#cross validation
valid_output = NULL
temp_output = data.frame(tau=0, nu=0, error =0)
for (i in 1:5) {
  tau_r = 25 * i
  for (j in 1:5) {
    nu_c = 50 * j
    try2  <- mLDA.cv(exprFinal_mLDA_10ysurvival, caseControlResponse, Z=NULL, fold=5, seed=1, tau=tau_r, alpha=0.9, nu=nu_c, d=2, nb=5)
    temp_output[1] = tau_r  
    temp_output[2] = nu_c  
    temp_output[3] = try2[["err.tot"]]/length(caseControlResponse)  
    valid_output = rbind(valid_output,temp_output)
    print(c(tau_r,nu_c))
  }
}
write.csv(valid_output,"survival_crossvalidation_mLDA.csv")
#try2  <- mLDA.cv(exprFinal_mLDA_10ysurvival, caseControlResponse, Z=NULL, fold=5, seed=1, tau=50, alpha=0.9, nu=150, d=2, nb=5)
#saveRDS(try2,file=paste("survival_crossvalidation_mLDA", ".rds", sep=""))
#try2 = readRDS(file=paste("survival_crossvalidation_mLDA", ".rds", sep=""))

#case-contral study using the mLDA by the strong signals from edgeR
exprFinal_mLDA_10ysurvival <- exprFinal_mLDA_10ysurvival[,colnames(exprFinal_mLDA_10ysurvival)%in%rownames(qlfT_sort)]
edgeR.genes <- rownames(qlfT_sort)
exprFinal_mLDA_10ysurvival <- exprFinal_mLDA_10ysurvival[,colnames(exprFinal_mLDA_10ysurvival)%in%edgeR.genes]
edgeR.set <- match(edgeR.genes, colnames(exprFinal_mLDA_10ysurvival))
edgeR.set <- edgeR.set[!is.na(edgeR.set)]
Xnew <- exprFinal_mLDA_10ysurvival[,edgeR.set]
match(c("BRCA1","BRCA2"), colnames(Xnew))

#try2_new  <- mLDA.pair(exprFinal_mLDA_10ysurvival, caseControlResponse, exprFinal_mLDA_10ysurvival, Z=NULL, Z.new=NULL, pair=c(1,0), strong.X.set=c(1:50), tau=50, alpha=0.8, nu=150, d=2, nb=5)   
try2_new  <- mLDA.pair(Xnew, caseControlResponse, Xnew, Z=NULL, Z.new=NULL, pair=c(1,0), strong.X.set=c(1:50,3128,2146), tau=52, alpha=0.7, nu=150, d=2, nb=5)   
saveRDS(try2_new,file=paste("survival_mLDA_V2", ".rds", sep=""))
try2_new <- readRDS(file=paste("survival_mLDA_V2", ".rds", sep=""))
write.csv(as.data.frame(try2_new[["network.nodes"]]),"survival_mLDA.csv")
write.csv(as.data.frame(try2_new[["MIset"]]),"survival_mLDA_strong.csv")
...
#####survival kaplan meier based on the prediction from mLDA     ####################
PredClass <- rep(0,length(caseControlResponse))
PredClass[which(try2_new[["PredClass"]]=="TRUE")] <- 1
prediction_error=length(which(PredClass!=caseControlResponse))/length(caseControlResponse)
Surv_TCGA_10_years$OSD <- PredClass
library(survival)
library(ggplot2)
library(survminer)
km_fit <- survfit(Surv(OS.time) ~ OSD, data=Surv_TCGA_10_years)
ggsurvplot(
  fit = km_fit, 
  xlab = "Survival Days", 
  ylab = "Overall survival probability")
...
Subtypes_analysis = subset(filterTCGA02, sampleID %in% Surv_TCGA_10_years$`rownames(exprFinal_mLDA_10ysurvival)`)
...
Surv_TCGA_10_years$OSD <- Subtypes_analysis$PAM50mRNAnature2012
km_fit <- survfit(Surv(OS.time) ~ OSD, data=Surv_TCGA_10_years)
ggsurvplot(
  fit = km_fit, 
  xlab = "Survival Days", 
