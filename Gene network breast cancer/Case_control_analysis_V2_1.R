...
setwd("C:/Users/Fengwei Yang/Desktop/YFW work/Others/Breast cancer")
...
#resLRT_Sig = subset(resLRT, pvalue<0.05)
resLRTOrdered <- resMFType[order(resMFType$pvalue),]
write.csv(as.data.frame(resLRTOrdered), file="Deseq_result.csv")

#edgeR.genes <- sapply(rownames(qlfT_sort), function(x) unlist(strsplit(x, "\\."))[[1]][1])
edgeR.genes <- rownames(qlfT_sort)
edgeR.genes <- as.character(case_control_edgeR_V2$Gene)
...
try2_new  <- mLDA.pair(Xnew, caseControlResponse, Xnew, Z=NULL, Z.new=NULL, pair=c(1,0), strong.X.set=c(1:50,927,625), tau=52, alpha=0.8, nu=0, d=2, nb=5)   
...
write.csv(casecontrol_mLDA_network,"casecontrol_mLDA_network_V2.csv")
...
GSEAdata <- case_control_edgeR_GSEA_KEGG_output[1:13,]
...
######################################################################
nodes_name <- rownames(as.data.frame(try2_new[["local.network_list"]][[3]]))
...
plot(mst,edge.arrow.size=1, vertex.color="gold",  vertex.size=5,
     vertex.frame.color="gray", vertex.label.color="black", 
     vertex.label.cex=0.8, vertex.label.dist=1, edge.curved=1,layout=layout.fruchterman.reingold,main="Network #2")

temp <- exprFinal_mLDA_cross[,colnames(exprFinal_mLDA_cross)%in%network_comparison$Casecontrol]
temp <- as.matrix(as.dist(cor(temp, method="pearson")))
temp[which(temp<=0.8)] <- 0
temp1 <- exprFinal_mLDA_cross[,colnames(exprFinal_mLDA_cross)%in%network_comparison$Casecontrol]
temp1 <- as.matrix(solve(cov(temp1)))
temp1[which(abs(temp1)<=1.3)] <- 0
temp1[which(temp==0)] <- 0
write.csv(temp1,"casecontrol_selected_network.csv")

###draw network of overlapping genes among 3 studies
#colnames(network_comparison) <- c("gene","net","Strong nodes","rank","gene1","net1","Strong nodes1","rank1",        
#                                  "gene2","net2","Strong nodes2","rank2")
nodes_name <- network_comparison$gene2[9:26]
temp <- exprFinal_mLDA_cross[,colnames(exprFinal_mLDA_cross)%in%nodes_name]
rownames(temp)<-1:dim(temp)[1]
XpairSel <- temp
library(igraph)
...
# Remove edges below absolute Pearson correlation 0.8
g <- delete_edges(g, E(g)[which(E(g)$weight<0.7)])
# Convert the graph adjacency object into a minimum spanning tree based on Prim's algorithm
mst <- mst(g, algorithm="prim")
# Plot the tree object
plot(mst,edge.arrow.size=1, vertex.color="gold",  vertex.size=5,
...
######################################################################
setwd("C:/Users/z021w783/Desktop/GRA/Other paper/Breast Cancer/Results/Comparison")
...
pathway_comparison <- as.matrix(pathway_comparison)

pv.out <- pathview(gene.data = pathway_comparison[,1], pathway.id = "04110",species = "hsa", 
