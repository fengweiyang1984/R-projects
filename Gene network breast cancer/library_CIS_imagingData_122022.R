...
mLDA.Kclass <-
function(X, y, X.new, Z=NULL, Z.new=NULL, COV=NULL, COR=NULL, K=3, strong.X.list=NULL, tau=20, alpha=0.5, nu=100, d=10, nb=10){
...
mLDA.cv <-
function(X, Y, Z=NULL, fold=5, seed=1, tau=50, alpha=0.9, nu=500, d=2, nb=5)
{
...
# 
 ...
 for(f in 1:fold)
  {  print(paste("fold=",f))   
	...
	if(K==2){
	  pair <- c(max(Y), min(Y))
	  tmp  <- mLDA.pair(X.m_new, Y.m, X.t_new, Z=Z.m_new, Z.new=Z.t_new, pair=pair, tau=tau, alpha=alpha, nu=nu, d=d, nb=nb)	
	}else if(K>2){
          tmp  <- mLDA.Kclass(X.m_new, Y.m, X.t_new, Z=Z.m_new, Z.new=Z.t_new, K=K, tau=tau, alpha=alpha, nu=nu, d=d, nb=nb)
        }
