...
IP_gglasso_calculation <- function(X,Y,Sigma_inv,lambda_lasso=0){
  library(gglasso)
  #parameters setting
  np <- dim(X)
...
IRLS_IP_gglasso <- function(x,times,event,Sigma_inv,lambda_lasso=0,max.iter=100,conv.eps=1e-6,INITIAL_BETA=0){
  store_gradient <- NULL
  ...
  for(iter in 1:max.iter){
    ...
    if(iter==1){
      ...
      lambda_lasso2 = 0.001
      output <- IP_gglasso_calculation(X1,z1,Sigma_inv=Sigma_inv,lambda_lasso=lambda_lasso1)
      beta <- output$beta_prediction
...
#calculate baseline for cox model fitting based on breslow method
#time/status is the time-to-event outcome
#X is the factors matrix
#B is the coefficients from model fitting
...
#calculate survival probability for cox model fitting 
###if it applied for newdata, X2 = newdata
#time2/status2 is the time-to-event outcome
#X2 is the factors matrix
#B2 is the coefficients from model fitting
Survival_Prob_est<- function(time2, status2, X2, B2){
  ...
  #res <- H0[match(data$ID, H0$ID),4]*exp(LP)
  return(res)
}

cindex_calcu <- function(time2, status2, X2, B2)
{ 
  n <- length(time2)
  ord <- order(time2,-status2)
  time <- time2[ord]
  status <- status2[ord]
  LP_indiv <- c(as.matrix(X2)%*%as.matrix(B2)) 
  LP_sample <- sum(colMeans(X2) * B2) 
  LP <- LP_indiv - LP_sample
  LP <- LP[ord]
