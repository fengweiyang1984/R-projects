...
#    Sset.order <- sort(Sset.tmp, decreasing=TRUE)
    ...
    result <- list(network.nodes_list=network.nodes_list, network.Omega_list=network.Omega_list, ISmatrix=ISmatrix.o, screenset=Sset, Fisher.matrix=Fisher.matrix, PredClass=predClass, MImatrix.cord=MImatrix.cord, MImatrix.name=MImatrix.name, conn_Array=conn_Array)
...
mLDA.pair <-
function(X, y, X.new, Z=NULL, Z.new=NULL, COV=NULL, COR=NULL, pair=c(1,2), tau=20, alpha=0.5, nu=100, d=10){
...
#	meanAvg.s.m <- cbind(ZmeanAvg.m0, meanAvg.s.m0)
	...
	result <- list(network.nodes=screenID, network.Omega=Omega.s, iffcond=iffcond.s_order, screenset=screenID2, FisherDR=FisherDR, PredClass=PredClass, MIset = meanDiff_firstTau, connMatrix = conn_matrix)
...
function(X, y, X.new, Z=NULL, Z.new=NULL, COV=NULL, COR=NULL, K=3, tau=20, alpha=0.5, nu=100, d=10){
    ...
    betaEffects.matrix <- matrix(0, p, (K*(K-1)/2))
	if(!is.na(Z)){
		ZEffects.matrix <- matrix(0, dim(as.matrix(Z))[2], (K*(K-1)/2))
	}
	inercept.vector <- rep(0, (K*(K-1)/2))
    ...
    for(k1 in 2:K){
   	 for(k in 1:(k1-1)){
   	    tmp <- mlogistic.pair(X, y, X.new, Z=Z, Z.new=Z.new, COV=COV, COR=COR, pair=c(k,k1), tau=tau, alpha=alpha, nu=nu, d=d)     
 		betaEffects.matrix[as.numeric(names(tmp$betaEffects)), ((k1-1)*(k1-2)/2+k)] <- tmp$betaEffects
		if(!is.na(Z)){
			ZEffects.matrix[, ((k1-1)*(k1-2)/2+k)] <- tmp$ZEffects
		}
		inercept.vector[((k1-1)*(k1-2)/2+k)] <- tmp$inercept
    ...
    nu.vec.tmp <- NULL
    for(k in 1:K){
         nu.vec.tmp <- c(nu.vec.tmp, betaEffects.matrix[which(betaEffects.matrix[,1]!=0), k])
    }

    nu.value <- sort(nu.vec.tmp, decreasing=TRUE)[nu]

    betaEffects.matrix[which(betaEffects.matrix<nu.value, arr.ind=TRUE)] <- 0
    Sset <- which(apply(betaEffects.matrix, 1, function(x) sum(abs(x)))!=0)
    betaEffects.matrix.o <- betaEffects.matrix[Sset, ]

	intercept.matrix <- matrix(rep(inercept.vector, n), dim(Z)[2], n, byrow=TRUE)


    Sset.tmp <- apply(betaEffects.matrix, 1, function(x) sum(abs(x)))[Sset]
    names(Sset.tmp) <- Sset
    Sset.order <- sort(Sset.tmp, decreasing=TRUE)

	if(!is.na(Z)){
		tmp <-  X[,Sset] %*% betaEffects.matrix.o + Z %*% ZEffects.matrix + intercept.matrix
	}else{
		tmp <-  X[,Sset] %*% betaEffects.matrix.o + intercept.matrix
	}
	predProb <- exp(tmp)/(1+exp(tmp))
	#predClass <- as.vector(as.numeric(predProb > 0.5))
	max.index.vec <- apply(predProb, 1, which.max)  
    ...
    result <- list(betaEffects.matrix=betaEffects.matrix.o, ZEffects.matrix=ZEffects.matrix, screenset=Sset, Fisher.matrix=Fisher.matrix, PredClass=predClass, MImatrix=MImatrix, conn_Array=conn_Array, Omega.cis.mat = Omega.cis.mat)
...
function(X, y, X.new, Z=NULL, Z.new=NULL, COV=NULL, COR=NULL, pair=c(1,2), tau=20, alpha=0.5, nu=100, d=10){
	...
	betaEff <- rep(0, dim(X)[2])
	for(pp in 1:dim(X)[2]){
		y12 <- y[which(y==pair[1] | y==pair[2])]
		y12 <- as.numeric(y12==pair[2])
		X12 <- as.matrix(X[which(y==pair[1] | y==pair[2]),])
		Z12 <- as.matrix(Z[which(y==pair[1] | y==pair[2]),])
		mylogit <- glm(y12 ~ X12[,1]+Z12, family = "binomial")
		betaEff[pp] <- mylogit$coefficients[2]
    }

	#meanDiff <- apply(as.matrix(X12[which(y12==0),]), 2, mean) - apply(as.matrix(X12[which(y12==1),]), 2, mean)
	betaEffRank <- order(-abs(betaEff))
	betaEff_firstTau <- betaEffRank[1:tau]
...
#	meanAvg <- (apply(as.matrix(X12[which(y12==0),]), 2, mean) + apply(as.matrix(X12[which(y12==1),]), 2, mean))/2
	...
	vis=rep(0, p)
	id <- betaEff_firstTau[1]
	scr <- betaEff_firstTau
	conn_matrix	<- matrix(0, tau, p)
    rownames(conn_matrix) <- betaEff_firstTau
	...
	while(!is.na(id)){
		...
		tau_id <- which(betaEff_firstTau==id)
		conn_matrix[tau_id, con_set] <- 1 
	...
	mylogit <- glm(y12 ~ X12[,screenID]+Z12, family = "binomial")
	betaEffects <- mylogit$coefficients[2:(length(screenID)+1)]
	ZEffects <- mylogit$coefficients[(length(screenID)+2):(length(screenID)+1+dim(Z)[2])]
	intercept <- mylogit$coefficients[1]


	betaEffectsRank <- order(-abs(betaEffects))
	betaEffects_firstNu <- betaEffectsRank[1:min(nu, length(betaEffects))]
	screenID2 <- betaEffects_firstNu

	Omega.s2 <- Omega[screenID2, screenID2]	
	diag(Omega.s2) <- apply(X[,screenID2], 2, var)


	tmp <-  X12[,betaEffects_firstNu] %*% betaEffects[betaEffects_firstNu] + Z12 %*% ZEffects + intercept
	predProb <- exp(tmp)/(1+exp(tmp))
	predClass <- as.vector(as.numeric(predProb > 0.5))


	result <- list(betaEffects=betaEffects, ZEffects=ZEffects, intercept=intercept, screenset=screenID2, PredClass=PredClass, MIset = betaEff_firstTau, connMatrix = conn_matrix, Omega.select=Omega.s2)
