...
#p is # of variables
n=1000      #sample size
p=1000      #parameters size
sep <- 100  # # of x_i in a structure
...
library(glmnet)
#lasso
temp <- glmnet(x=X,y=y, alpha = 1,lambda=0.085, family = binomial(link = "logit"), relax=T,intercept = T)
...
#ridge
temp <- glmnet(x=X,y=y, alpha = 0,lambda=0.05, family = binomial(link = "logit"), relax=T,intercept = T)
glm_ridge <- as.matrix(temp[["beta"]])
glm_ridge_bo <- temp[["a0"]]
#elastic net
temp <- glmnet(x=X,y=y, alpha = 0.5,lambda=0.075, family = binomial(link = "logit"), relax=T,intercept = T)
