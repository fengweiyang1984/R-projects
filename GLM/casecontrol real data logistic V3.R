...
##################################################################################################
##################################################################################################
setwd("C:/Users/Fengwei Yang/Desktop/YFW work/GRA/Disertation/Real data example")
...
IRLS.IP.gglasso.crossvalidation <-
  function(X2, y2, n, sepn =5,lambda = 5){
    valid_result <- NULL
    for (k in seq(0.01, 0.3, length.out = 30)) {
    valid_output = NULL
    for (tt in 1:sepn) {
      ...
      upb2 = min(ceiling((n-178)/sepn*tt),n-178)
      data_set1 = X2[1:178,]
      data_set2 = X2[-1:-178,]
      ...
      library("glmnet") 
      #temp_lasso <- glmnet(x=X_input,y=y_input, alpha = 1,lambda=k, family = binomial(link = "logit"), relax=T,intercept = F)
      #glm_mine <- as.matrix(temp_lasso[["beta"]])
      #temp_EN <- glmnet(x=X_input,y=y_input, alpha = 0.5,lambda=k, family = binomial(link = "logit"), relax=T,intercept = F)
      #glm_mine <- as.matrix(temp_EN[["beta"]])
...
temp.lasso.cvfit <- cv.glmnet(x=X_input,y=y_input, family = "binomial", type.measure = "class", alpha = 1)
lamb <- temp.lasso.cvfit$lambda.1se ###or lamb <- temp.lasso.cvfit$lambda.min ### or
#> temp.lasso.cvfit$lambda.1se
#[1] 0.04629028
temp.EN.cvfit <- cv.glmnet(x=X_input,y=y_input, family = "binomial", type.measure = "class", alpha = 0.5)
lamb <- temp.lasso.cvfit$lambda.1se ###or lamb <- temp.lasso.cvfit$lambda.min ### or 
#> temp.lasso.cvfit$lambda.1se
#[1] 0.04217798
...
length(which(glm_mine1!=0))

library("glmnet") 
#lasso
temp.lasso.cvfit <- cv.glmnet(x=X_input,y=y_input, family = "binomial", type.measure = "class", alpha = 1)
lamb <- temp.lasso.cvfit$lambda.1se ### or lamb <- temp.lasso.cvfit$lambda.min
temp.lasso <- glmnet(x=X_input,y=y_input, alpha = 1, lambda=lamb, family = "binomial",intercept = F)
glm_laaso <- as.matrix(temp.lasso[["beta"]])
glm_laaso_bo <- temp[["a0"]]

#elastic net
temp <- glmnet(x=X_input,y=y_input, alpha = 0.5,lambda=0.1, family = binomial(link = "logit"), relax=T,intercept = F)
...
IRLS.IP.crossvalidation <-
  function(X2, y2, n, sepn =5,k = 0.03){
      valid_output = NULL
      for (tt in 1:sepn) {
        #### Preparing the training & testing dataset
        lowb1 = ceiling(178/sepn*(tt-1))+1
        ...
        y<-y_Train
        #glm_mine <- IRLS_IP_gglasso(X=X,y=y,lambda_lasso=k)
        #glm_mine <- IRLS_IP_sparsegl(X=X,y=y)
        library("glmnet") 
        temp.lasso.cvfit <- cv.glmnet(x=X,y=y, family = "binomial", type.measure = "class", alpha = 0.5)
        lamb <- temp.lasso.cvfit$lambda.1se ### or lamb <- temp.lasso.cvfit$lambda.min
        temp.lasso <- glmnet(x=X,y=y, alpha = 0.5, lambda=lamb, family = "binomial")
        #glm_mine <- as.matrix(temp.lasso[["beta"]])
        p_prediction <- predict(temp.lasso, s='lambda.min', newx=X_test, type="response")
        #pred.lasso01 <- predict(temp.lasso, s='lambda.min', newx=X_test, type="class")
        ...
        #y_prediction[which(p_prediction>=0.5)] <- 1
        #y_prediction[which(p_prediction<=0.5)] <- 0
        #temp <- cbind(y_prediction,y_Test)
        valid_output <- rbind(valid_output,p_prediction)
    }
    return(valid_output)
  }
source("C:/Users/z021w783/Desktop/GRA/Disertation/Code/GLM Paper final library/GLM Iterative Proximal (IP) Algorithm Sparse Regression for logistic V_gglasso5.R")
source("C:/Users/z021w783/Desktop/GRA/Disertation/Code/GLM Paper final library/GLM Iterative Proximal (IP) Algorithm Sparse Regression for logistic V_sparsegl5.R")
valid_result <- IRLS.IP.crossvalidation(X2=X_input, y2=y_input, n=nrow(X_input),sepn =5)
write.csv(valid_result,"valid_result.csv")


###IRLS_IP_gglasso
####load Crossvalidation_results from Crossvalidation_results.excl 
temp <- matrix(c(0, 0, 0, 1, 1, 1), ncol=3, byrow=TRUE)
colnames(temp) <- c('thresholding','tpr','fpr')
temp <- as.data.frame(temp)
for (i in 1:9){
  thresholding = i/10
  temp1 <- Crossvalidation_results
  temp1$IRLS_IP_gglasso_pred <- rep(0,nrow(temp1))
  temp1$IRLS_IP_gglasso_pred[which(temp1$IRLS_IP_gglasso>=(thresholding))]<- 1
...
ggplot(temp, aes(x = fpr, y = tpr)) + geom_line(size = 1.2) + geom_abline(intercept = 0, slope = 1) +  theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))
...
for (i in 1:9){
  thresholding = i/10
  temp1 <- Crossvalidation_results
  temp1$IRLS_IP_sparsegl_pred <- rep(0,nrow(temp1))
  temp1$IRLS_IP_sparsegl_pred[which(temp1$IRLS_IP_sparsegl>=(thresholding))]<- 1
...
for (i in 1:9){
  thresholding = i/10
  temp1 <- Crossvalidation_results
  temp1$glm_laaso_pred <- rep(0,nrow(temp1))
  temp1$glm_laaso_pred[which(temp1$Lasso>=(thresholding))]<- 1
...
for (i in 1:9){
  thresholding = i/10
  temp1 <- Crossvalidation_results
  temp1$glm_EN_pred <- rep(0,nrow(temp1))
  temp1$glm_EN_pred[which(temp1$EN>=(thresholding))]<- 1
...
#ggplot(ROC_data, aes(x = fpr, y = tpr, color = method)) + geom_line(size = 1) + geom_abline(intercept = 0, slope = 1) +  theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))
colnames(Crossvalidation_results)
ROC_data <- paste(Crossvalidation_results[,2],Crossvalidation_results[,3],
                            Crossvalidation_results[,3],Crossvalidation_results[,4])
ggplot(Crossvalidation_results, aes(d = true_outcome, m = EN)) + geom_roc() 
...
pv.out <- pathview(gene.data = pathway_comparison[,1], pathway.id = "05224",species = "hsa", 
                   out.suffix = "Poisson_BC", kegg.native = T,same.layer = F,key.pos = "bottomright",  sign.pos = "topright",
