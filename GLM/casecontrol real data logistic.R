...
library("glmnet") 
#lasso
temp <- glmnet(x=X_input,y=y_input, alpha = 1,lambda=0.05, family = binomial(link = "logit"), relax=T,intercept = F)
...
for (i in 1:10){
  ...
  temp1$IRLS_IP_gglasso_pred[which(temp1$IRLS_IP_gglasso>=(1-thresholding))]<- 1
  ...
  temp1$IRLS_IP_gglasso_pred[which(temp1$IRLS_IP_gglasso<=(thresholding))]<- 0
...
for (i in 1:10){
  ...
  temp1$IRLS_IP_sparsegl_pred[which(temp1$IRLS_IP_sparsegl>=(1-thresholding))]<- 1
  ...
  temp1$IRLS_IP_sparsegl_pred[which(temp1$IRLS_IP_sparsegl<=(thresholding))]<- 0
...
for (i in 1:10){
  ...
  temp1$glm_laaso_pred[which(temp1$glm_laaso>=(1-thresholding))]<- 1
  ...
  temp1$glm_laaso_pred[which(temp1$glm_laaso<=(thresholding))]<- 0
...
for (i in 1:10){
  ...
  temp1$glm_EN_pred[which(temp1$glm_EN>=(1-thresholding))]<- 1
  tpr = length(which(temp1$glm_EN_pred[which(temp1$true_outcome==1)]==1)) / length(which(temp1$true_outcome == 1))
  temp1$glm_EN_pred<- 1
  temp1$glm_EN_pred[which(temp1$glm_EN<=(thresholding))]<- 0
