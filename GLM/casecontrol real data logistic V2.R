...
y_prediction_summary <- y_prediction
...
for (i in 1:20){
  ...
  temp1$IRLS_IP_gglasso_pred[which(temp1$IRLS_IP_gglasso>=(0.7+thresholding))]<- 1
  tpr = length(which(temp1$IRLS_IP_gglasso_pred[which(temp1$true_outcome==1)]==1)) / length(which(temp1$true_outcome == 1))
  temp1$IRLS_IP_gglasso_pred<- 1
  temp1$IRLS_IP_gglasso_pred[which(temp1$IRLS_IP_gglasso<=(0.04-thresholding))]<- 0
...
for (i in 1:20){
  ...
  temp1$IRLS_IP_sparsegl_pred[which(temp1$IRLS_IP_sparsegl>=(0.6-thresholding))]<- 1
  tpr = length(which(temp1$IRLS_IP_sparsegl_pred[which(temp1$true_outcome==1)]==1)) / length(which(temp1$true_outcome == 1))
  temp1$IRLS_IP_sparsegl_pred<- 1
  temp1$IRLS_IP_sparsegl_pred[which(temp1$IRLS_IP_sparsegl<=(0.1-thresholding))]<- 0
...
for (i in 1:20){
  ...
  temp2 <- rbind(temp2,c(thresholding,tpr,fpr))
}
ggplot(temp2, aes(x = fpr, y = tpr)) + geom_line(size = 1.2) + geom_abline(intercept = 0, slope = 1) +  theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))
...
for (i in 1:20){
  ...
  temp1$glm_EN_pred[which(temp1$glm_EN>=(0.5-thresholding))]<- 1
  ...
  temp3 <- rbind(temp3,c(thresholding,tpr,fpr))
}
ggplot(temp3, aes(x = fpr, y = tpr)) + geom_line(size = 1.2) + geom_abline(intercept = 0, slope = 1) +  theme(axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))

ROC_data <- rbind(temp,temp1,temp2,temp3)
