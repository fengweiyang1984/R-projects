...
repeat{
  ...
  for (j in 1:5) {
source("C:/Users/z021w783/Desktop/GRA/Disertation/Code/GLM Paper final library/GLM Iterative Proximal (IP) Algorithm Sparse Regression for logistic V_gglasso4.R")
source("C:/Users/z021w783/Desktop/GRA/Disertation/Code/GLM Paper final library/GLM Iterative Proximal (IP) Algorithm Sparse Regression for logistic V_sparsegl4.R")
...
############################################################
  ...
  temp_result[1,9] <- length(which(y_prediction[which(y==1)] != 1)) #false negative for outcome prediction
  temp_result[1,10] <- length(which(y_prediction[which(y==1)] == 1))/length(which(y == 1)) #sensitivity for outcome prediction
  temp_result[1,11] <- length(which(y_prediction[which(y==0)] == 0))/length(which(y == 0)) #specificity for outcome prediction
...
#lasso_result <- 3
  ...
  temp_result[1,7] <- mean(abs(T1-T2))  #MAE
  p_prediction <- exp(X%*%T1)/(1+exp(X%*%T1))
  #p_prediction <- exp(cbind(1,X)%*%c(intercept[,1],T1))/(1+exp(cbind(1,X)%*%c(intercept[,1],T1)))
  #y_prediction <- rbinom(dim(X)[1], 1, p_prediction)
  y_prediction <- rep(2,dim(X)[1])
  y_prediction[which(p_prediction>=0.5)] <- 1
  y_prediction[which(p_prediction<=0.5)] <- 0
...
#ridge_result <- 4
  ...
  temp_result[1,7] <- mean(abs(T1-T2))  #MAE
  p_prediction <- exp(X%*%T1)/(1+exp(X%*%T1))
  #p_prediction <- exp(cbind(1,X)%*%c(intercept[,2],T1))/(1+exp(cbind(1,X)%*%c(intercept[,2],T1)))
  #y_prediction <- rbinom(dim(X)[1], 1, p_prediction)
  y_prediction <- rep(2,dim(X)[1])
  y_prediction[which(p_prediction>=0.83)] <- 1
  y_prediction[which(p_prediction<=0.17)] <- 0
  temp_result[1,8] <- length(which(y_prediction[which(y==0)] != 0)) #false positive for outcome prediction
  temp_result[1,9] <- length(which(y_prediction[which(y==1)] != 1)) #false negative for outcome prediction
  temp_result[1,10] <- length(which(y_prediction[which(y==1)] == 1))/length(which(y == 1)) #sensitivity for outcome prediction
  temp_result[1,11] <- length(which(y_prediction[which(y==0)] == 0))/length(which(y == 0)) #specificity for outcome prediction
...
#EN_result <- 5
  ...
  temp_result[1,7] <- mean(abs(T1-T2))  #MAE
  p_prediction <- exp(X%*%T1)/(1+exp(X%*%T1))
  #p_prediction <- exp(cbind(1,X)%*%c(intercept[,3],T1))/(1+exp(cbind(1,X)%*%c(intercept[,3],T1)))
  #y_prediction <- rbinom(dim(X)[1], 1, p_prediction)
  y_prediction <- rep(2,dim(X)[1])
  y_prediction[which(p_prediction>=0.5)] <- 1
  y_prediction[which(p_prediction<=0.5)] <- 0
